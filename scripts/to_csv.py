...
HEADER = ('notice_date', 'initiated_date', 'employer', 'employer_street',
          'employer_city', 'employer_state', 'employer_zip',
          'employer_representative', 'employer_representative_phone',
          'employer_representative_title', 'employer_representative_email',
          'union_name', 'union_street', 'union_city', 'union_state',
          'union_zip', 'union_representative', 'union_representative_phone',
          'union_representative_title', 'union_representative_email',
          'affected_location_city', 'affected_location_state',
          'affected_location_zip', 'expiration_date', 'naics',
          'industry', 'bargaining_unit_size', 'establishment_size',
          'notice_submitted_by', 'category', 'healthcare_related')

HEADER_LOOKUP = {'e-street': 'employer_street',
...
for filename in sys.argv[1:]:
    with open(filename) as f:
        ...
        slugged_fields = (field.lower().replace(' ', '_') for field in reader.fieldnames)
        normalized_fields = [field if field in header_set else HEADER_LOOKUP[field]
                             for field in slugged_fields]
        assert set(normalized_fields) - header_set == set(), print(set(normalized_fields) - header_set)
